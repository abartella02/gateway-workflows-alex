...
from .Nitrogen_form import GenericFormTemplate
...
# chaos will ensue.
@route(app, '/Nitrogen/Nitrogen_endpoint')
@util.workflow_permission_required('Nitrogen_page')
@util.exception_catcher
@util.ui_secure_endpoint
def Nitrogen_Nitrogen_page():
...
@route(app, '/Nitrogen/form', methods=['POST'])
@util.workflow_permission_required('Nitrogen_page')
@util.exception_catcher
@util.ui_secure_endpoint
def Nitrogen_Nitrogen_page_form():
    form = GenericFormTemplate()
    # Remove this line if your workflow does not need to select a configuration
    form.configuration.choices = util.get_configurations(default_val=True)
    ...
    return redirect(url_for('NitrogenNitrogen_Nitrogen_page'))
