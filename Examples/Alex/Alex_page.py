...
from .Alex_form import GenericFormTemplate
...
# chaos will ensue.
@route(app, '/Alex/Alex_endpoint')
@util.workflow_permission_required('Alex_page')
...
@route(app, '/Alex/form', methods=['POST'])
@util.workflow_permission_required('Alex_page')
@util.exception_catcher
@util.ui_secure_endpoint
def Alex_Alex_page_form():
    form = GenericFormTemplate()
    # Remove this line if your workflow does not need to select a configuration
    form.configuration.choices = util.get_configurations(default_val=True)
    ...
    return redirect(url_for('AlexAlex_Alex_page'))
